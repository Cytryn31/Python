...
class Canny:
    ...
    def __init__(self,imname,sigma,thresHigh = 50,thresLow = 10):
        ...
        #original_img = cv2.imread('1_1.png', cv2.CV_LOAD_IMAGE_GRAYSCALE)
        ...
        #Image.fromarray(self.theta).show()
        self.theta[x0,y0] = 0
        ...
        x,y = self.theta.shape  # to construct temp
        ...
        for i in range(x):
            for j in range(y):
                if self.theta[i,j] == 0:
                    ...
                    temp.putpixel((j,i),(0,0,0))
       
        print 'teta',self.theta
        print 'theta mean', np.mean(self.theta)
        print 'temp',temp 
        temp.show()
    ...
    def gaussFilter(self,sigma,window = 3):
        ...
        print 'heere',window
        c0 = window // 2 #is the centre of the kernel
        print 'heeere',c0
        for x in range(window): # creates kernel using Gaussian function
            for y in range(window):
                r = hypot((x-c0),(y-c0)) #calculates magnitude of the centre pixel. x^2 + y^2
                val = (1.0/2*pi*sigma*sigma)*exp(-(r*r)/(2*sigma*sigma))
                kernel[x,y] = val
        return kernel / kernel.sum() # ask kolmo xke esegue questa operation
