...
Orientation.py
Description:
    The intent of this function is to draw lines so to follow the ridges. The
    angle at each pixel is computed using the first part of the Canny edge detection
    algorithm. The angles are then used to determine the angle of the line that passes
    in each 8x8 window in the image.
